...
\end{array}\right]
\end{equation}
where $\omega_{0}$ and $\omega_{1}$ are complex numbers called probability amplitudes, because when their module is squared they represent a probability or a probability density function associated with the system being in a certain state. When a system is in a mixture of pure states, meaning $\omega_{0}$ and $\omega_{1}$ are both differemt than $0$, it is in a state known as a superposition. 
