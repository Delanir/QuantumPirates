...
When working with a compound system wich holds $n$ particles represented by the quantum state $\vert \psi \rangle$ we often want to be able to separate its components $\psi_{0}, \psi_{1},.. \psi_{n-1}$. This way we use the following notations as equivalent $\vert \psi \rangle = \vert \psi_{0}, \psi_{1}, ... \psi_{n-1} \rangle = \vert \psi_{0}\rangle \vert \psi_{1}\rangle ... \vert \psi_{n-1} \rangle =  \vert \psi_{0}\rangle \otimes \vert \psi_{1}\rangle \otimes ... \otimes \vert \psi_{n-1}$\cite{Rieffel2011}
