...
\caption{Playing the second round of the Pirate Game with 3 players without performing an intermediate measuring. }
...
Building the expected payoff functionals for the three player sittuation must take into account the subgames created when the proposal fail. These utility functions will represent the degree of satisfaction for each pirate after game by atributiong a real number to a measurement performed to the system, as in Equation \ref{eq:quantum_game_definition_payoff_func} (Section \ref{sec:background_quantum_game_theory}). 
