...
By hipothesys, the way we set up our system may originate different results, so when we have a $3$ player system, we will try to analyse the subgame with $2$ players taking into account the original system. In the classical version of the Pirate Game, the problem with $2$ players is equivalent to a subproblem with $2$ players of a problem with more players where previous captains are killed. 
...
\begin{center}
\begin{tabular}{cc}
  a)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/CC1.PNG}}
    & b)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/CC2.PNG}} \\
  c)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/CD1.PNG}}
    & d)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/CD2.PNG}} \\
  e)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/DC1.PNG}}
    & f)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/DC2.PNG}} \\
  g)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/DD1.PNG}}
    & h)\putindeepbox[7pt]{\includegraphics[scale=0.46]{compareluders/DD2.PNG}} \\
\end{tabular}
\caption{Comparrison of the expected utilities when measuring and not measuring between steps 1 and 2 of a 3 player game (the player 1 will be the player 2 in the 3 player game, and the player 2 will be the player 3 in the 3 game situation).}
\label{tab:luderscomp}
\end{center}
 \end{table}

We set up a system with $3$ players and in the step $1$ the three players chose the operators $o_{10},o_{21},o_{31}$ producing the outcome $CDD$. 
...
In one situation we will use the L\"{u}der's Rule, after the voting. This is the equivalent of performing a measurement on the system. In the other situation the two players will act on step $2$. The results for this experiment (Table \ref{tab:luderscomp}), allow us to conclude that not measuring the system affects the expected utilities for the players in two of the four possible analysed outcomes. In this case the act of measuring leaves us with the same payoffs one would get in a classic situation with $2$ players game, the equilibrium here is $CD$ outcome. Without the intermediate measuring step the outcome will depend on the $\gamma$ - the initial coeficient used to set up the system. For example:

\begin{itemize}
\item $\gamma = 0$, we have the same results as in the original problem. The Nash equilibrium is unique and it is $CD$.
\item $\gamma = \frac{\pi}{2}$, the equlibrium is $DC$, as the payoffs for the $CC$ outcome are $(-200, 0.5)$, and the outcome $DD$ will present $(100, 0)$ for the expected utilities of the players.
\item $\gamma = \frac{\pi}{4}$, all possible outcomes will be Nash equlibria. As we can see in Table \ref{tab:hate_myself}, no player can improve her payoff by unilateraly changing her strategy.
...
\caption{Normal form representation of the second step in a 3 player game with an entanglement coeficient of $\frac{\pi}{4}$, where there isn't an intermediate measuring. }
\label{tab:hate_myself}
\end{table}
\end{center}


If we adopt the version where we measure the system in between states, we can formulate the payoff functionals as:
 \begin{equation}
\begin{split}
E_{1}(\vert\psi_{fin}\rangle, \alpha_{1})=\alpha_{1}\times(\vert\langle000\vert\psi_{fin}\rangle\vert^{2} + \vert\langle100\vert\psi_{fin}\rangle\vert^{2}
...
\end{equation}
 ...
 \begin{equation}
\begin{split}
E_{2}(\vert\psi_{fin}\rangle, \alpha_{2})=\alpha_{2}\times(\vert\langle000\vert\psi_{fin}\rangle\vert^{2} + \vert\langle100\vert\psi_{fin}\rangle\vert^{2}
+ \vert\langle010\vert\psi_{fin}\rangle\vert^{2}
+ \vert\langle001\vert\psi_{fin}\rangle\vert^{2}
 ) - \\
 + 0.5\times(\vert\langle111\vert\psi_{fin}\rangle\vert^{2} + \vert\langle110\vert\psi_{fin}\rangle\vert^{2}
+ \vert\langle101\vert\psi_{fin}\rangle\vert^{2}
+ \vert\langle011\vert\psi_{fin}\rangle\vert^{2}
 )
\end{split}
\end{equation}
 ...
 \begin{equation}
\begin{split}
E_{3}(\vert\psi_{fin}\rangle, \alpha_{3})=\alpha_{3}\times(\vert\langle000\vert\psi_{fin}\rangle\vert^{2} + \vert\langle100\vert\psi_{fin}\rangle\vert^{2}
+ \vert\langle010\vert\psi_{fin}\rangle\vert^{2}
+ \vert\langle001\vert\psi_{fin}\rangle\vert^{2}
 ) - \\
 + 0.5\times(\vert\langle111\vert\psi_{fin}\rangle\vert^{2} + \vert\langle110\vert\psi_{fin}\rangle\vert^{2}
